"""Phase 3b: Phase 1.5's backtest but with the Optuna-tuned XGBoost.

Same 7 features, same train/val/test split as Phase 1.5; the only change
is the XGBoost hyperparameters now reflect the Phase-3 Optuna optimum
(pinned as defaults in ``src/models.py``). Output to a separate
``results/03b_tuned_xgboost/`` so Phase 1.5's frozen numbers stay
reproducible; comparison between the two ``metrics.parquet`` files
isolates the tuning effect.

Run with:
    .venv/bin/python -m notebooks.personb.03b_tuned_xgboost
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "03b_tuned_xgboost"
