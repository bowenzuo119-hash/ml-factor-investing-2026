"""Phase 11: full 3-layer sector neutrality (Layer 2 + Layer 3 combined).

Phase 2 tried Layer 2 (sector-relative target) alone -- the model
trained on sector-deviations but the portfolio still picked globally,
so the two halves disagreed and Sharpe fell.

Phase 10 tried Layer 3 (sector-neutral portfolio, k=10 per sector)
alone -- the portfolio was sector-balanced but the model wasn't trained
for that view, so it gave up the sector-tilt alpha it had learned.

Phase 11 = Layer 2 AND Layer 3 together. Model trains on (return - sector
mean), and the portfolio picks top-k/bottom-k per sector. Now the model
and the portfolio have a consistent "within-sector" view: the model
identifies within-sector winners, and the portfolio takes exactly those
within-sector positions.

Hypothesis: with internal consistency between train target and portfolio
construction, Phase 11 should recover the alpha that Phase 2 and Phase 10
each gave up in isolation. This is the framework's full 3-layer prescription
(Layer 1 = sector-relative features, already in factors.py).

Output to ``results/11_layer2_plus_layer3/``.
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "11_layer2_plus_layer3"
