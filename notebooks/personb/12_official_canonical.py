"""Phase 12: OFFICIAL CANONICAL CONFIGURATION (as of 2026-05-23).

The framework's full 3-layer sector neutrality stack (Phase 11 recipe)
applied to the extended 2003-2024 returns panel (Phase 9b window):

* 13 features (Phase 8 fundamentals stack)
* Tuned XGBoost hyperparameters from Phase 3's Optuna study
* target_kind="sector_relative" (Layer 2)
* k_per_sector=10 with sector_map=load_sector_map() (Layer 3)
* 2003-2024 panel (extends Phase 8's 2005-2024 by 2 years, matches
  framework test window's 2024 end)
* v0.3.0 engine (block-gated refit)

The Phase 11 recipe on 2005-2024 already lifted XGBoost long-OOS Sharpe
from +0.91 to +1.09 and NN long-OOS from +0.52 to +1.31. The extra two
years of training history should tighten significance further.

This is the new headline number for the final report. Phase 8 (dollar-
neutral), Phase 10 (Layer 3 only), and Phase 11 (Layer 2+3 on 2005-2024)
remain available as decomposition / robustness checks.

Output to ``results/12_official_canonical/``.
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "12_official_canonical"
...
PANEL_FILE = Path(__file__).resolve().parents[2] / "data" / "processed" / "returns_spliced_2003_2024.parquet"
