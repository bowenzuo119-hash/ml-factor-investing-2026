"""Phase 5c: 8 features + lag features (x_{t-1}, x_{t-2}).

Implements Framework section 3.5: since XGBoost processes each row
independently, temporal patterns must be pre-digested into scalar columns.
For each of the 8 base features we add a 1-month lag and a 2-month lag,
giving the model 24 columns total instead of 8. The model can then learn
"rising momentum predicts X" vs "falling momentum predicts Y".

Uses the SAME tuned XGBoost defaults from Phase 3c -- not re-tuned for
the bigger feature set, intentionally. We want to isolate the effect of
adding lags from the effect of hyperparameter changes. If lags clearly
help here, a follow-up tuning pass on the 24-feature panel is justified;
if they don't help, no point.

Output to ``results/05c_lag_features/``.
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "05c_lag_features"
INCLUDE_FEATURES = ("mom", "rev", "mvol", "ivol", "log_mktcap",
                    "bm", "ep", "dvol")
LAG_MONTHS = (1, 2)  # x_{t-1} and x_{t-2} per base feature -> 24 cols total
...
def main() -> int:
    ...
    print(f"Building feature panel ({len(INCLUDE_FEATURES)} base + "
          f"{len(INCLUDE_FEATURES) * len(LAG_MONTHS)} lag features)...")
