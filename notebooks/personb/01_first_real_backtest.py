"""Phase 1 of Person B's roadmap: the first real backtest on 2005-2024.

Drives the three models (Lasso, XGBoost, NN) end-to-end against Bowen's
backtest engine on the full Framework sample, captures predictions for
the prediction-quality metrics (OOS R^2, IC), and saves all artefacts
under results/01_first_real_backtest/ for the final report.

Run with:
    .venv/bin/python -m notebooks.personb.01_first_real_backtest

What it produces (in results/01_first_real_backtest/):
  metrics.parquet              -- one row per (model, window) with all metrics
  cumulative_returns.png       -- net cumulative return per model on the test window
  drawdowns.png                -- drawdown curves per model
  per_model_results.pkl        -- pickled BacktestResult objects (for re-use)
  predictions.parquet          -- raw model predictions, long-format

Sample split (Framework section 7.2)
  Training 2005-2015, Validation 2016-2018, Testing 2019-2024.
  The walk-forward engine uses a sliding window so the literal split is
  fuzzy at edges; we report metrics on the full out-of-sample range
  AND on the strict 2019-2024 test subset for the report's headline numbers.
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "01_first_real_backtest"
...
def main() -> int:
    ...
    print("Building feature panel via build_feature_panel...")
