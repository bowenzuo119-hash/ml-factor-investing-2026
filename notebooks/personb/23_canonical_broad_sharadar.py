...
# Retuned XGBoost hyperparameters from Phase 23a (Optuna on broad
# Sharadar panel). Loaded at runtime from the best_params.json so this
# file always uses the latest tune.
...
if _BEST_PARAMS_FILE.exists():
    with open(_BEST_PARAMS_FILE) as _f:
        _bp = _json.load(_f)["by_model"]
    RETUNED_XGB_PARAMS = _bp["xgboost"]["best_params"]
    RETUNED_LASSO_PARAMS = _bp.get("lasso", {}).get("best_params", {"alpha": 1e-4})
    RETUNED_NN_PARAMS = _bp.get("nn", {}).get("best_params", {})
else:
    # Phase 23a hasn't run yet; fall back to Phase 19's params so this
    # script doesn't crash if someone runs it out of order.
    RETUNED_XGB_PARAMS = {
        "n_estimators": 50, "max_depth": 5, "learning_rate": 0.00886,
        "subsample": 0.94837, "colsample_bytree": 0.65052,
        "min_child_weight": 12, "reg_alpha": 0.64281, "reg_lambda": 5.42577,
    }
    RETUNED_LASSO_PARAMS = {"alpha": 1e-4}
    RETUNED_NN_PARAMS = {}
...
RESULTS_DIR = (
    Path(__file__).resolve().parents[2]
    / "results" / "23_canonical_broad_sharadar"
)
# Layer 3: k longs and k shorts within EACH GICS sector. Start at k=5
# for consistency; Phase 23b sweeps to confirm/replace.
K_PER_SECTOR = 5
TARGET_KIND = "sector_relative"
INCLUDE_FEATURES = ("mom", "rev", "mvol", "ivol", "log_mktcap",
                    "bm", "ep", "dvol",
                    "roe", "roa", "de", "asset_growth", "accruals")
# Pre-computed by Bowen's Block B/C:
RETURNS_FILE = (
...
def main() -> int:
    ...
    # 1. Load data --------------------------------------------------------
    print("=" * 72)
    print("Phase 23: FINAL CANONICAL (broad Sharadar universe, retuned)")
    print("=" * 72)
    ...
    print(f"  retuned XGBoost params from Phase 23a: {RETUNED_XGB_PARAMS}")
