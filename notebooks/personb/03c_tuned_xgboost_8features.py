"""Phase 3c: Phase 3b's backtest extended to 8 features (adds dvol).

Same train/val/test split as Phase 3b; the changes are:
* feature panel now includes dvol (Bowen's load_dollar_volume_monthly,
  yfinance close x volume, 21d trailing mean, month-end-sampled),
* XGBoost defaults re-tuned on the 8-feature panel (validation
  R^2 = +0.02125, slightly lower than the 7-feature +0.02178 -- a sign
  that dvol may add as much noise as signal at validation time).
Output to ``results/03c_tuned_xgboost_8features/`` to keep 03b frozen
for comparison.

Run with:
    .venv/bin/python -m notebooks.personb.03c_tuned_xgboost_8features
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "03c_tuned_xgboost_8features"
