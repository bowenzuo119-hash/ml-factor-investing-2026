"""Phase 9: 13-feature canonical backtest on the extended 2003-2025 sample.

Identical to Phase 8 v0.3.0 (same features, same tuned XGBoost defaults,
same engine) except the input panel is the wider 2003-2025 one produced
by `notebooks/personb/freeze_extended_panel.py`. With train_window=120
and START=2003-01, the first OOS prediction lands at 2013-01-31,
extending the long-OOS window from 10 years (2015-2024) to 13 years
(2013-2025).

The extra data ought to tighten the Sharpe estimate and push the t-stat
from ~2.85 to ~3.5+ on the long-OOS window, which makes every
significance test pass by an even wider margin.

Output to ``results/09_extended_window_2003_2025/``.
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "09_extended_window_2003_2025"
INCLUDE_FEATURES = ("mom", "rev", "mvol", "ivol", "log_mktcap",
                    "bm", "ep", "dvol",
                    "roe", "roa", "de", "asset_growth", "accruals")
PANEL_FILE = Path(__file__).resolve().parents[2] / "data" / "processed" / "returns_spliced_2003_2025.parquet"
