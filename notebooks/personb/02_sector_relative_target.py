"""Phase 2: same 7-feature backtest but with sector-relative target (Layer 2).

Identical to ``01b_with_value_factors.py`` except each model is instantiated
with ``target_kind="sector_relative"``. The model still consumes the same
features, but its training target is now (next-period return) minus the
per-(date, sector) cross-sectional mean -- Framework section 3.2 Layer 2,
which makes the learning objective sector-neutral by construction.

Two notebook-level changes vs Phase 1.5:
  * features ARE passed to the engine WITH the 'sector' column intact
    (the model's _split_X drops it before the underlying estimator;
    leaving it on X is required for the sector-relative demean).
  * each model: ``target_kind="sector_relative"``.

Output: results/02_sector_relative_target/ -- compare against
        results/01b_with_value_factors/metrics.parquet to isolate Layer 2.

Run with:
    .venv/bin/python -m notebooks.personb.02_sector_relative_target
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "02_sector_relative_target"
...
def main() -> int:
    ...
    # models need it to compute per-(date, sector) demeans inside fit().
    # Their internal _split_X drops sector before the underlying
    # estimator sees it, so the column never becomes a predictive feature.
