"""Phase 1.5: Phase 1's backtest with B/M and E/P added to the feature set.

Identical to ``01_first_real_backtest.py`` except the feature panel now
includes the two Fama-French value factors via Sharadar SF1 (Bowen's
2026-05-22 ``load_fundamentals`` work). Output goes to a separate
``results/01b_with_value_factors/`` directory so Phase 1's frozen
numbers stay reproducible; the two ``metrics.parquet`` files can be
diff'd to isolate the value-factor contribution.

Run with:
    .venv/bin/python -m notebooks.personb.01b_with_value_factors

Sample split (Framework section 7.2)
  Training 2005-2015, Validation 2016-2018, Testing 2019-2024. Same as
  Phase 1; only the feature set changes.
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "01b_with_value_factors"
