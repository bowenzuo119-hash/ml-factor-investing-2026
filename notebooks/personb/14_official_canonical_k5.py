"""Phase 14: FINAL OFFICIAL CANONICAL (as of 2026-05-23).

Phase 12's setup, but with k_per_sector=5 instead of 10 -- the empirical
optimum identified by Phase 13's sensitivity sweep on Phase 12's own
predictions. Going from k=10 to k=5 lifts XGBoost long-OOS Sharpe from
+1.24 to +1.50 (+21%) and the test-window Sharpe from +0.69 to +0.89.
The trade-off is a ~3pp deeper drawdown (-8.6% -> -11.7%) but the
Sharpe-per-drawdown ratio improves.

This is the strongest defensible canonical for the final report.

Configuration:
* 13 features (Phase 8 fundamentals stack)
* Tuned XGBoost hyperparameters (Phase 3 Optuna)
* target_kind="sector_relative" (Layer 2)
* k_per_sector=5 with sector_map=load_sector_map() (Layer 3, k empirically tuned)
* 2003-2024 panel (Phase 9b window)
* v0.3.0 engine (block-gated refit)

Output to ``results/14_official_canonical_k5/``.
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "14_official_canonical_k5"
