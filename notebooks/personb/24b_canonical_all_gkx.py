...
# Phase 24b: 16 features (chmom + maxret + mom36m all added). XGBoost
# params from Phase 24b's retune on the 16-feature Q-filtered panel.
# Lasso/NN params still from Phase 23a (less likely to benefit).
import json as _json
_XGB_PARAMS_FILE = (
    Path(__file__).resolve().parents[2]
    / "results" / "24b_retune_xgb_all_gkx" / "best_params.json"
)
_OTHER_PARAMS_FILE = (
    Path(__file__).resolve().parents[2]
...
RESULTS_DIR = (
    Path(__file__).resolve().parents[2]
    / "results" / "24b_canonical_all_gkx"
)
# k=20: same as Phase 23g for direct apples-to-apples comparison.
K_PER_SECTOR = 20
TARGET_KIND = "sector_relative"
# 13 -> 16 features: chmom + maxret + mom36m added (all GKX top-12).
# Note: INCLUDE_FEATURES is not enforced here — the driver passes the
# entire features panel (minus 'sector') to the engine. Bowen's panel
# has 17 columns (16 features + sector). The engine uses all features
# automatically.
INCLUDE_FEATURES = ("mom", "rev", "mvol", "ivol", "log_mktcap",
                    "bm", "ep", "dvol",
                    "roe", "roa", "de", "asset_growth", "accruals",
                    "chmom", "maxret", "mom36m")  # 16 features total
