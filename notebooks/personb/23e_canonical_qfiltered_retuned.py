...
# Retuned XGBoost from Phase 23d (Optuna on Q-FILTERED Sharadar panel).
# Lasso/NN stay from Phase 23a since they weren't retuned on Q-filtered
# data (XGBoost was the leak-prone one).
import json as _json
_XGB_PARAMS_FILE = (
    Path(__file__).resolve().parents[2]
    / "results" / "23d_retune_xgb_qfiltered" / "best_params.json"
)
_OTHER_PARAMS_FILE = (
    Path(__file__).resolve().parents[2]
...
if _XGB_PARAMS_FILE.exists():
    ...
    # Phase 23d hasn't run -- fall back to Phase 23a's tune (works but
    # on the unfiltered panel; user should re-run after 23d completes).
    RETUNED_XGB_PARAMS = {
        "n_estimators": 100, "max_depth": 6, "learning_rate": 0.01321,
        "subsample": 0.553, "colsample_bytree": 0.556,
        "min_child_weight": 23, "reg_alpha": 1.695, "reg_lambda": 5.275,
    }
if _OTHER_PARAMS_FILE.exists():
    with open(_OTHER_PARAMS_FILE) as _f:
        _other = _json.load(_f)["by_model"]
    RETUNED_LASSO_PARAMS = _other.get("lasso", {}).get("best_params",
                                                       {"alpha": 1e-4})
    RETUNED_NN_PARAMS = _other.get("nn", {}).get("best_params", {})
else:
...
RESULTS_DIR = (
    Path(__file__).resolve().parents[2]
    / "results" / "23e_canonical_qfiltered_retuned"
)
# k=20: Phase 23c's k-sweep WITH Q-filter showed k=15-30 is the plateau;
