"""Phase 8: 13-feature canonical backtest.

Phase 3c's setup with 5 additional Sharadar-SF1-sourced quality factors:
  - roe (return on equity, ART)
  - roa (return on assets, ART)
  - de (debt-to-equity, ARQ)
  - asset_growth (YoY change in total assets, ARQ)
  - accruals (Sloan-style: (TTM netinc - TTM ncfo) / assets, ART)

XGBoost defaults re-tuned on 13 features (Optuna, validation
R^2 = +0.0210; see results/03_xgboost_tuning/best_params.json).
Pattern shift vs the 8-feature tune: shallower trees (max_depth 4 -> 3),
much more aggressive L1 (reg_alpha 0.44 -> 0.79) so the model uses L1
for feature selection across the wider panel.

Output to ``results/08_extended_fundamentals/`` to keep Phase 3c frozen
for comparison.
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / "results" / "08_extended_fundamentals"
